...
if($connection){
    $query_res = pg_query($connection, "select * from \"User_info\" where username='". $user . "';");
    if(!$query_res){
...
if($connection){
  ...
  if(pg_num_rows($username_query_res) > 0 && $username != $row['username']){
    $userNameErr = "A user with that username already exists.";
}
if(pg_num_rows($email_query_res) > 0 && $email != $row['email']){
        ...
        <div id='login-box' class="container box">
            ...
            <form method="post" action="edit_profile.php">
              ...
              <!--  <div class="form-group">
                ...
                <!-- Sign UP button -->
                <button class="button btn btn-lg btn-block btn-secondary" id="login-button"> Save </button>
            </form>
        </div>
    </div>
